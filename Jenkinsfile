...
pipeline {
    agent any
     ...
     environment {
         ...
         CredentialID="4ef87fe8-89c5-4892-98f9-0ea0c101697c"
         port="8080"         
     }
     //start of stages
     stages {
         ...
         stage('Clone Code') {
              steps {
                  checkout changelog: false, poll: false, scm: [$class: 'GitSCM', branches: [[name: "master"]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: "$TargetDir"]], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/Deepak33/JFrog-assignment.git']]]
     ...
     stage('Build Docker image') {
         steps {
             ...
             [[ $(docker ps | grep integral-admin | wc -l) -gt 0 ]] && docker stop $(docker ps | grep $image_name | awk '{print $1}') && docker rm $(docker ps -a | grep $image_name | awk '{print $1}')
